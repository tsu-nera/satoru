# HRV分析レポート（簡易版）

## 測定情報

- **測定日時**: 2026-01-10 16:08:50
- **測定時間**: 15.3分
- **データポイント数**: 1133点
- **平均心拍数**: 74.4 bpm

---

## 主要HRV指標

### 時間領域

| 指標 | 値 | 評価 |
|:-----|---:|:-----|
| **SDNN** | 53.1 ms | ✅ 良好 |
| **RMSSD** | 26.5 ms | ⭕ 中程度 |
| **pNN50** | 7.5% | ⭕ 中程度 |

### 周波数領域

| 指標 | 値 | 評価 |
|:-----|---:|:-----|
| **LF Power** | 0.01 ms² | 低周波成分 (0.04-0.15 Hz) |
| **HF Power** | 0.00 ms² | 高周波成分 (0.15-0.4 Hz) |
| **LF/HF比** | 23.61 | ⚠️ 交感神経優位 |

---

## 解釈

**自律神経バランス**: LF/HF比が23.61で、交感神経優位（ストレス状態）です。

**心拍変動レベル**: SDNN 53.1msは良好な心拍変動を示しています。

**副交感神経活動**: RMSSD 26.5msは中程度の副交感神経活動を示唆しています。

---

## 可視化

### 周波数領域分析
![Frequency Domain](hrv_frequency.png)

### Poincaréプロット
![Poincare Plot](hrv_poincare.png)

### 時間領域分析
![Time Domain](hrv_time.png)

---

## 技術情報

- **解析ライブラリ**: NeuroKit2
- **計算されたHRV指標**: 91個
- **データソース**: SelfLoops HRV

---

*生成日時: 2026-01-12 13:28:40*
