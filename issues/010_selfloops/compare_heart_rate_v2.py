#!/usr/bin/env python3
"""
Muse心拍数 vs SelfLoops心拍数 比較分析スクリプト v2

既存のloader/ecg実装を活用したリファクタリング版。
新しいlib.loaders.selfloopsとlib.sensors.ecgを使用。

Usage:
    python compare_heart_rate_v2.py --eeg <CSV_PATH> --hrv <CSV_PATH> [--output <OUTPUT_DIR>]
"""
...
def synchronize_heart_rates(muse_hr_data, selfloops_hrv_data, interval_sec=1):
    """
    MuseとSelfLoopsの心拍数を時間軸で同期

    Args:
        muse_hr_data: Muse心拍数データ（dict）
        selfloops_hrv_data: SelfLoops HRVデータ（dict、get_hrv_data()の戻り値）
    ...
    selfloops_hr = selfloops_hrv_data['hr']  # 既に計算済み
    selfloops_start = selfloops_hrv_data['session_start']

    if selfloops_start is None:
        # フォールバック: Museの開始時刻を使用
        print('  警告: SelfLoopsの開始時刻が取得できませんでした。Museの開始時刻を使用します。')
        selfloops_start = muse_timestamps.min()

    selfloops_timestamps = selfloops_start + pd.to_timedelta(selfloops_hrv_data['time'], unit='s')
...
def generate_comparison_report(aligned_df, stats_dict, minute_df, output_path):
    ...
    report = f"""# Muse vs SelfLoops 心拍数比較分析レポート (v2)
...
**Note**: このレポートは新しいlib.loaders.selfloopsとlib.sensors.hrvを使用したv2版です。
...
### 今後の拡張性
- `lib.sensors.hrv.analysis`を使用することで、NeuroKit2の高度なHRV解析が可能
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Muse vs SelfLoops 心拍数比較分析 (v2 - 新loader実装版)'
    )
    parser.add_argument(
        '--eeg',
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=60.0,
        help='ウォームアップ期間（秒）- EEG/HRV測定開始直後のデータを除外'
    )
    ...
    print('='*60)
    print('Muse vs SelfLoops 心拍数比較分析 (v2)')
    print('='*60)
    print()

    # 1. データ読み込み
    print(f'Loading EEG: {args.eeg.name}')
    eeg_df = load_mind_monitor_csv(args.eeg, filter_headband=False, warmup_seconds=args.warmup)
    print(f'EEGデータ形状: {eeg_df.shape[0]} 行 × {eeg_df.shape[1]} 列')

    print(f'Loading HRV: {args.hrv.name}')
    hrv_df = load_selfloops_csv(args.hrv, warmup_seconds=args.warmup)
    print(f'HRVデータ形状: {hrv_df.shape[0]} 行 × {hrv_df.shape[1]} 列')

    # セッション開始時刻の表示
    session_start = hrv_df.attrs.get('session_start')
    if session_start:
        print(f'SelfLoops測定開始時刻: {session_start.strftime("%Y-%m-%d %H:%M:%S")}')
    print()
    ...
    # 3. SelfLoops HRVデータ取得
    print('取得中: SelfLoops HRVデータ...')
    selfloops_hrv_data = get_hrv_data(hrv_df)
    print(f'  データポイント数: {len(selfloops_hrv_data["hr"])}')
    print(f'  平均心拍数: {np.mean(selfloops_hrv_data["hr"]):.2f} bpm')
    print()

    # 4. データ同期
    print('同期中: 心拍数データ...')
    aligned_df = synchronize_heart_rates(muse_hr_data, selfloops_hrv_data, interval_sec=1)
    ...
    # 7. 可視化
    print('生成中: 比較プロット...')
    plot_comparison(aligned_df, stats_dict, minute_df, args.output / 'hr_comparison_v2.png')
    print()

    # 8. レポート生成
    print('生成中: 比較レポート...')
    generate_comparison_report(aligned_df, stats_dict, minute_df, args.output / 'HR_COMPARISON_REPORT_V2.md')
    print()
    ...
    print(f'  - HR_COMPARISON_REPORT_V2.md')
    print(f'  - hr_comparison_v2.png')
