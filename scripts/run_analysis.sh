...
#
# Usage: ./run_analysis.sh [CSV_FILE_PATH] [OUTPUT_DIR]
#
# Arguments:
...
if [ $# -eq 0 ]; then
    ...
    if [ -z "$CSV_FILE" ]; then
        ...
        echo ""
        echo "Usage: $0 [CSV_FILE_PATH] [OUTPUT_DIR]"
        echo ""
        echo "例："
        echo "  $0                                                    # 最新CSVをtmp/に出力"
        echo "  $0 data/mindMonitor_2025-11-04--16-59-52.csv         # 指定CSVをtmp/に出力"
        echo "  $0 data/mindMonitor_2025-11-04--16-59-52.csv output/ # 指定CSVをoutput/に出力"
        exit 1
    fi
...
python "$PROJECT_ROOT/scripts/generate_report.py" --data "$CSV_FILE" --output "$OUTPUT_DIR"
